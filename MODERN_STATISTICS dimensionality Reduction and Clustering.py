...
def calculate_kband_mask(k, n):
    ...
    :param k: k-band rank
    :param n: dim
    :return: mask to create matrix with one's in the right places in the matrix (tri)
    """
    mask = np.zeros((n, n))
    for i in range(n):
        top = i + k
...
def calculate_kband_mult(S, L, k, n):
    ...
    :param k:
    :param n:
    :return: put in i'th col the mult of some indexes
    """
...
def avoid_nurmerical_break(diag_L):
    """
    if some element in matrix is very close - just make it 1e-20
    this is the way to stand with the numerical issues
    :param diag_L:
    :return:
    """
...
def generateRandomData(k, n):
    """
    return diagonal matrix L with mask we defined before
    :param k:
    :param n:
    :return:
    """
...
def generateRandomDataS(n):
    """
    return matrix S (muls of L L transpose)
    :param n:
    :return:
    """
    # PSD
...
def calc_g_step(S, L, k, n, mask):
    """
    return the gradient of our formula that we describe in the pdf
    """
...
def GD_total(S, k, searching_steps=5,opt_K_flag=False,opt_K=[],return_grad=False,iter_total=10000,iter_per_lr=100,learn_K=False):
    """
    our simulated train - not for using
    :return:
    """
...
def GD_solve(S, k, searching_steps=5,iter_total=10000):
    """
    do GD on our problem to find the solotion and stops with some condition on the gradient
    :return:
    """
    n=S.shape[0]
...
def search_optimal_lr(L, S, k, search_s,range_max=1,min_range=0.1):
    """
    return the optimal lr to the next step (2 steps among 5 lr that we check)
...
def init_random_L(lr, S, k, init_amount, iter_per_init):
    """
    init random matrices and do gd tio each one of them, and return the matrix with the lowest value of the function (tr(s)-..)
    """
    n = S.shape[0]
...
def calc_func(S, L):
    """
    just math , return the value of function
    :param S:
    :param L:
    :return:
    """
...
def stability_fix_L(L1):
    """
    if element in diagonal is negative - we correct the element to STABILITY_EPS (we keep the matrix be PSD)
    :param L1:
    :return:
    """
...
def synthize_values(n,k):
    """
    simulate matrix S that is really close to the optimal matrix, then random some matrices with presicion of the threshold we need to find
    :param n:
    :param k:
    :return:
    """
...
def calc_iter_step(x):
    """
    calc how many iters to do for each lr we get
    :param x:
    :return:
    """
...
#mat = np.ndarray([[2,-1-0],[-1,2,0],[0,-1,2]], dtype='int8')
